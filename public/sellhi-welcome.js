/*
 * SellHi — Welcome (app-layer; demo.html stays pristine).
 *
 * Two coordinated layers, shown on EVERY login for everyone (so people can revisit):
 *   • BACK  (#sh-welcome, this file): full-screen branded "track" screen — the
 *     positioning tagline + the 5-chapter spine, so the modules read as one story.
 *   • FRONT (#sh-welcome-overlay, from demo.html): the "Guided concept demo" card,
 *     with a "Don't show again" checkbox that opts out of the WHOLE welcome per device.
 * The 6-spoke mark on both comes from sellhi-logo.js.
 */
(function () {
  "use strict";
  var OPTOUT = "sellhi_welcome_optout";
  var shownBack = false, shownFront = false, wrapped = false;
  ...
  function backDismiss(go) {
    hideBack();
    if (go) { try { if (typeof window.showPhase === "function") window.showPhase("p1"); } catch (e) {} }
  }
  function onKey(e) { if (e.key === "Escape") backDismiss(false); }
  ...
  function showBack() {
    ...
    document.getElementById("shw-go").onclick = function () { backDismiss(true); };
    document.getElementById("shw-skip").onclick = function () { backDismiss(false); };
    document.addEventListener("keydown", onKey);
  }

  // ---- Front card (demo's #sh-welcome-overlay) gate ----
  ...
  function wrapClose() {
    ...
    window.shWelcomeClose = function (tour) {
      ...
      try {
        ...
        if (cb && cb.checked) { localStorage.setItem(OPTOUT, "1"); out = true; }
        else localStorage.removeItem(OPTOUT);
      } catch (e) {}
      // Opting out, or launching the tour, clears the back track screen so nothing
      // sits over the app. "Explore on my own" reveals the track screen for a beat.
      if (out || tour) hideBack();
  ...
  function ensure() {
    if (!shownBack) { shownBack = true; if (!optedOut()) showBack(); }
    var ov = document.getElementById("sh-welcome-overlay");
    if (ov) {
      injectCheckbox(document.getElementById("sh-welcome-card"));
      wrapClose();
      if (!shownFront) { shownFront = true; ov.style.display = optedOut() ? "none" : "flex"; }
    }
    return !!ov;
  }
  ...
  function boot() {
    var n = 0;
    var t = setInterval(function () { var ok = ensure(); if ((ok && wrapped) || ++n > 100) clearInterval(t); }, 100);
  }
